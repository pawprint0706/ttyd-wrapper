...
if [[ "$DRY" == "0" && -t 0 ]]; then
    ...
    if ask_yn "  Enable login?" N; then
        ENABLE_AUTH=1
        # The credential is rendered into the unit file: a double-quote breaks
        # ExecStart quoting and $ / % trigger systemd expansions - re-ask.
        while :; do
            read -r -p "    Username: " _lu || true
            read -r -s -p "    Password: " _lp || true; echo
            CRED="${_lu:-}:${_lp:-}"
            case "$CRED" in
                *'"'*|*'$'*|*%*) echo "    [WARN] avoid \", \$ and % in the credentials (systemd-unsafe) - re-enter" >&2; continue ;;
            esac
            break
        done
