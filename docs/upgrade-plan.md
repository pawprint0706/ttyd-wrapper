...
**설치 UX (대화형 + 사전점검)**
- 세 설치 스크립트가 실행 시 세션유지(Unix)·HTTPS·로그인 사용 여부를 **대화형으로 질문**(Enter=기본값). 비대화형(파이프)·`--dry`는 환경변수/Configuration 값으로 대체.
- **선택 기반 패키지 사전점검**: 선택에 필요한 `ttyd`(+세션유지 선택 시 `tmux`)가 없으면 설치를 시작하지 않고 설치 명령을 안내한 뒤 종료. HTTPS 선택 시 인증서 파일 존재도 확인. Windows는 세션유지 질문 없음(D=b)이고 ttyd 동봉이라 패키지 점검 없음.
- **검증**: `ask_yn` 단위테스트(y/n/빈값 기본), `--dry` 렌더가 선택 반영, 패키지·인증서 미존재 시 안내+exit 1 실측.
