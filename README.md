...
#### 세션 유지의 알려진 한계 (psmux × LocalSystem 서비스)

psmux 세션은 서비스가 뜨는 게 아니라 웹 터미널에 **첫 접속할 때** 만들어진다(부팅 직후 `psmux ls`가 비어 있는 게 정상). 여기에 더해 서비스가 LocalSystem으로 돌기 때문에 아래 한계가 따른다 — psmux가 세션 서버 프로세스를 띄우는 주체가 시스템 계정이라는 귀결이다.

- **웹 셸에서 `exit`하면 세션도 끝난다.** 마지막 pane 셸이 종료되면 세션(과 서버)이 소멸한다. 연결 유지는 *연결 단절*에 적용되는 기능이고, 재접속하면 새 세션이 만들어진다. 탭 닫기·네트워크 끊김은 detach라 세션이 유지된다.
- **일반(비관리자) 셸의 `psmux ls`에 서비스 세션이 보이지 않는다.** psmux 클라이언트는 세션 서버 PID의 생존을 `OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION)`로 확인하는데, LocalSystem이 띄운 서버 프로세스에는 일반 사용자 컨텍스트에서 이 호출이 거부되어 클라이언트가 살아있는 서버를 "죽은 것"으로 오판해 목록에서 뺀다(뜬금없이 등록 파일까지 지울 수 있으니 비관리자 `psmux ls`를 습관적으로 돌리지 않는 게 좋다). **관리자 권한 PowerShell**의 `psmux ls` / `psmux attach -t ttyd`에서는 정상적으로 보인다.
- 데스크톱 셸에서 psmux 명령을 돌리면 사용자 권한 warm 서버가 생겨, 이후 웹에서 만들어지는 세션이 사용자 소유로 떠 ls에 보이는 경우가 생긴다. 문제가 해소된 것처럼 보이지만 우연이므로, 데스크톱 쪽 psmux 사용은 세션 확인용(관리자 권한)으로만 두는 게 안전하다.
