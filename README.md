...
>
> **참고 — 붙여넣기(⎄) 버튼**: Clipboard API는 보안 컨텍스트(HTTPS 또는 localhost)에서만 동작한다. LAN IP로 HTTP 접속 시 ⎄ 버튼은 브라우저 종류와 무관하게 동작하지 않는다 — 이때는 터미널에 포커스 후 **소프트 키보드의 네이티브 붙여넣기**(길게 눌러 붙여넣기)를 사용하면 된다. 복사(⎘)는 폴백(`execCommand`)으로 HTTP에서도 동작한다.

## 7. 추후 개발 예정

- [ ] **인증** — ttyd `-c user:pass` 기본 인증 연동 및 서비스 스크립트 옵션화
- [ ] **HTTPS** — `-S/-C/-K` SSL 옵션 지원 (공용망 노출 시 필수). 적용 시 붙여넣기(⎄) 버튼도 모든 브라우저에서 활성화됨 (iOS Safari는 시스템 확인 팝업 추가)
